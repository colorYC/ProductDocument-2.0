...
主叫发起呼叫后，被叫会收到 onCallItemAdd 回调事件，并通过回调带上来的 JCCallItem 中的 `video <http://developer.juphoon.com/portal/reference/ios/Classes/JCCallItem.html#//api/name/video>`_ 属性以及 `direction <http://developer.juphoon.com/portal/reference/ios/Classes/JCCallItem.html#//api/name/direction>`_  属性值 JCCallDirectionIn 判断是视频呼入还是语音呼入，此时可以调用下面的接口进行应答，**语音通话只能进行语音应答**
::
...
::
    ...
    - (IBAction)voiceCall:(id)sender {
        ...
        // 调用接口发起语音呼叫
        [JCManager.shared.call call:_numberField.text video:false extraParam:@"iOS_Test"];
    }
...
::
    ...
    {
        // 应答通话
        [JCManager.shared.call answer:[self getActiveCall] video:false];
    }
